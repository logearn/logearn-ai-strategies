...
const FIELD_DESC = {
  ...
  'post_buy_max_drawdown_pct': '买入之后最大回撤（%） = (initial_mcap - min_mcap) / initial_mcap × 100，min_mcap 是 call 记录的买入之后市值最低点；未跌破买入价时按 0 计，不产生负数',
  ...
  'v_breakout_volume_recent_still_below_cost_line': '【最近生效V转】跌破成本线后，到快照时刻是否仍未站回（1=仍在成本线下，0=已收复）。用于暴露右删失：这类样本 break_cost_line_min 是缺失的，若不单独标记就会被静默剔除出统计，而它们恰是最差的一批',
  ...
  'v_breakout_volume_recent_signal_to_buy_min': '信号新鲜度（分钟）= 买入时刻 − signalTime；衡量执行延迟，对应 PVP 策略里的 vFresh 判定',
