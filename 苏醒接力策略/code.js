// 苏醒接力策略：Pump / four.meme 发射 + 10分钟内苏醒信号逐个放量 + 最新苏醒在成本线上
...
const prevVol = prevWake ? num(prevWake.current_volume) : 0
...
const checks = [
  ...
  ['10分钟内多苏醒', !!prevWake && inWindow > 1 && gap <= WINDOW, `${inWindow}个/间隔${gap}s`, '>1且<=600s'],
  ['放量>前一个', !!prevWake && latestVol > prevVol, `${latestVol.toFixed(1)}>${prevVol.toFixed(1)}`, '当前>前一个'],
